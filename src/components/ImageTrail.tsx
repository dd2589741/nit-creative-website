...
class ImageTrailVariant1 {
  ...
  constructor(container: HTMLElement) {
    ...
    this.images = [...this.DOM.el.querySelectorAll<HTMLElement>('.content__img')].map(img => new ImageItem(img));
...
class ImageTrailVariant2 {
  ...
  showNextImage() {
    ...
    this.imgPosition = this.imgPosition < this.imagesTotal - 1 ? this.imgPosition + 1 : 0;
    const img = this.images[this.imgPosition];
    if (!img || !img.DOM.el || !img.DOM.inner) return;
...
const ImageTrail: React.FC<ImageTrailProps> = ({ items = [], variant = 1 }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      // Clean up window resize listener on each item
      instance.images.forEach((img: any) => {
  ...
  return (
    <div className="content" ref={containerRef}>
      {items.map((url, i) => (
        <div className="content__img animate-pulse" key={i}>
