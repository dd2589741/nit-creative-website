...
class App {
  ...
  onTouchUp(e: any) {
    this.isDown = false;
    if (e && this.start !== undefined) {
      const x = e.changedTouches ? e.changedTouches[0].clientX : e.clientX;
      const dragDistance = Math.abs(this.start - x);
      if (dragDistance < 6) {
        const rect = this.renderer.gl.canvas.getBoundingClientRect();
        const clickXInCanvas = x - rect.left;
        const normalizedX = (clickXInCanvas / this.screen.width) - 0.5;
        const clickViewportX = normalizedX * this.viewport.width;
        
        let clickedMedia = null;
        let minDistance = Infinity;
        if (this.medias) {
          this.medias.forEach((media: any) => {
            const dist = Math.abs(media.plane.position.x - clickViewportX);
            if (dist < minDistance) {
              minDistance = dist;
              clickedMedia = media;
            }
  ...
  update() {
    ...
    if (this.medias && this.medias.length && this.onActiveIndexChange) {
      ...
      if (closestMedia) {
        // Calculate original index mapping modulo original length
        const halfLength = this.medias.length / 2;
        const originalIndex = closestMedia.index % halfLength;
        if (originalIndex !== this.activeIndex) {
  ...
  addEventListeners() {
    ...
    // Only listen to scroll and interactions inside the container bounds
    this.container?.addEventListener('mousewheel', this.boundOnWheel, { passive: false });
    this.container?.addEventListener('wheel', this.boundOnWheel, { passive: false });
    this.container?.addEventListener('mousedown', this.boundOnTouchDown);
    this.container?.addEventListener('touchstart', this.boundOnTouchDown);

    // Track drags on the window so that if cursor slips outside the box, it still scrolls nicely
...
export default function CircularGallery({
  ...
  // Keep callback and config parameters in refs to prevent unnecessary re-renders of the WebGL canvas
  ...
  useEffect(() => {
    ...
    // Re-run setup ONLY if items list or bend structure strictly change, avoiding touch/hover/active-index triggers
  }, [itemsSerialized, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase]);
  return (
    <div
